...
resource "tls_locally_signed_cert" "squid" {
  cert_request_pem      = tls_cert_request.squid.cert_request_pem
...
resource "tls_cert_request" "squid" {
  private_key_pem = tls_private_key.squid.private_key_pem
  subject {
    common_name  = "squid"
    country      = "US"
    organization = "JonCorpIncLLC"
  }
  ip_addresses = ["127.0.0.1", split("/", var.squid_ip)[0]]
}
